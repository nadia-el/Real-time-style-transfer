...
# import tensorflow.contrib.slim as slim  # deprecated in tf2
...
def padding2d(x, p_h=1, p_w=1, pad_type='REFLECT', name='pad2d'):
    if pad_type == 'REFLECT':
        return tf.pad(tensor=x, paddings=[[0, 0], [p_h, p_h], [p_w, p_w], [0, 0]], mode='REFLECT', name=name)
...
def deconv2d(x, k, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02, padding_='SAME', output_size=None,
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        output_shape = [tf.shape(input=x)[0], h_output, w_output, k]
...
def upsampling2d(x, size=(2, 2), name='upsampling2d'):
    with tf.compat.v1.name_scope(name):
        ...
        return tf.image.resize(x, size=(size[0] * shape[1], size[1] * shape[2]), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        if is_train is True:
            ...
            mean = tf.compat.v1.get_variable('moving_mean', params_shape, tf.float32,
                                   initializer=tf.compat.v1.constant_initializer(0.0, tf.float32), trainable=False)
...
def instance_norm(x, name='instance_norm', mean=1.0, stddev=0.02, epsilon=1e-5):
    with tf.compat.v1.variable_scope(name):
        ...
        mean, variance = tf.nn.moments(x=x, axes=[1, 2], keepdims=True)
